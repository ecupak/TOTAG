...
void main()
{
...
#ifdef FULLSCREEN
	window = glfwCreateWindow( SCRWIDTH, SCRHEIGHT, "Temple of the Absent God", glfwGetPrimaryMonitor(), 0 );
#else	
	window = glfwCreateWindow(SCRWIDTH, SCRHEIGHT, "Temple of the Absent God", 0, 0);
	// Make game appear on second monitor.
	int monitorCount_{ -1 };
	GLFWmonitor** monitors{ glfwGetMonitors(&monitorCount_) };
	if (monitorCount_ != NULL && monitorCount_ > 1)
	{
		int xPos, yPos, width, height;
		glfwGetMonitorWorkarea(monitors[1], &xPos, &yPos, &width, &height);
		//glfwSetWindowPos(window, xPos + (width >> 1) - (SCRWIDTH >> 1), yPos + (height >> 1) - (SCRHEIGHT >> 1));
	}
#endif
...
#endif
#endif
	// [Credit] Remi for the upper FPS limit.
	float deltaTime = 0;
	...
	{
		bool shouldTick = (timer.elapsed() > 1.0f / targetFPS);
		if (shouldTick)
		{
			deltaTime = min(0.1f, timer.elapsed());
			timer.reset();
			app->Tick(deltaTime);
		}
